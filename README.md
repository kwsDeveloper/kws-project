...
> 생성된 문서는 각 프로젝트 폴더에 HTML로 저장되며 자동으로 GitHub에 push됩니다.  
> PDF가 필요하면 탐색기에서 HTML 파일 더블클릭 → `Ctrl+P` → PDF로 저장

### 기존 문서 참조 (새 대화창에서)

새 Claude Code 대화창을 열었을 때 이전에 만든 문서를 참조하는 방법:

| 방법 | 설명 |
|------|------|
| **자동 참조** | 해당 프로젝트 폴더를 Claude Code로 열고 "인수인계문서 읽어봐줘" 요청 → 폴더 안의 HTML 문서를 찾아 읽음 |
| **파일 첨부** | 채팅창에 HTML 파일을 드래그하여 직접 첨부 |

> 자동 참조는 Claude Code가 열려 있는 **프로젝트 폴더 안의 문서만** 읽을 수 있습니다.
